...
let heading start finish =
  print_endline (" HEADING: start:" ^ Ahoy.dump_coord start ^ " finish:" ^ Ahoy.dump_coord finish);
  let sx, sy = start in
  let fx, fy = finish in
  (sx - fx, sy - fy)
...
let rotate_waypoint direction degrees from =
  ...
  in
  print_endline (" ROTATED: to: " ^ Char.escaped !Ahoy.heading ^ ": " ^ string_of_int degrees ^ "[" ^ string_of_int ((start + (360 / degrees)) mod 4) ^ "] " ^ Ahoy.dump_coord foo);
  foo

let move_ship toward from =
  let _, distance = toward in
  let cx, cy = !ship in
  let x, y = (heading from !ship) in
